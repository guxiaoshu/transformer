...
class FeedForward(nn.Module):
    ...
    # 把原来的 ReLU 换成 SwiGLU：SwiGLU(x) = W3( SiLU(W1 x) ⊙ (W2 x) )。
    # SwiGLU 用门控机制，比 ReLU 表达更强，是 LLaMA / PaLM 等大模型的标准 FFN。
    # 注意 SwiGLU 有 3 个线性层（vs ReLU 的 2 个），参数量约多 50%。
    # 
    ...
    def forward(self, x: torch.Tensor, verbose: bool = False) -> torch.Tensor:
        ...
        value = self.w2(x)
        out = self.w3(self.dropout(gate * value))
        return out
